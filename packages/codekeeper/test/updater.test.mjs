...
import { resolveNpmCliPath, runLatestUpdate } from "../src/updater.mjs";

test("the latest-release bootstrap resolves a version then runs that exact package", async () => {
  const calls = [];
  const runner = createRecordingRunner((call) => {
    calls.push(call);
    return calls.length === 1 ? result('"1.4.2"\n') : result();
  });
  ...
  assert.deepEqual(calls[0].args, [
    "/trusted/lib/node_modules/npm/bin/npm-cli.js", "view", "codekeeper@latest", "version", "--json"
  ]);
...
test("the latest-release bootstrap rejects invalid registry versions before execution", async () => {
  const runner = createRecordingRunner(() => result('"latest"\n'));
