...
export function renderPolicy(policySource, {
  ...
  let policy;
  try {
    policy = upgradePolicy(policyOverride ? structuredClone(policyOverride) : JSON.parse(policySource));
  } catch (cause) {
...
export function renderAssistantWorkflow(template, { sourceRepository, sourceCommit, ownerRequests }) {
  if (count(template, "OWNER/REPOSITORY") !== 3 || count(template, "FULL_COMMIT_SHA") !== 3) {
  ...
  if (!/owner_requests: (?:true|false)/.test(template)) {
    throw new InstallerError("Bundled assistant workflow has no owner-request control.", { code: "WORKFLOW_RENDER_INVALID" });
  }
