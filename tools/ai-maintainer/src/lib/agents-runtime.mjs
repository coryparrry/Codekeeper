...
export function structuredOutputType(mode, schema) {
  ...
  return {
    type: "json_schema",
    name: `ai_maintainer_${mode}_result`,
    strict: true,
...
export async function runConfiguredAgent({
  ...
  apiKey = process.env.AI_MAINTAINER_MODEL_API_KEY,
  sdkLoader = () => import("@openai/agents")
}) {
  if (!apiKey || !String(apiKey).trim()) {
    throw new Error("AI_MAINTAINER_MODEL_API_KEY is required for the configured model provider");
  }
  ...
  const traceApiKey = process.env.AI_MAINTAINER_TRACE_API_KEY?.trim();
  if (tracing.enabled) {
    if (!traceApiKey) {
      throw new Error("AI_MAINTAINER_TRACE_API_KEY is required when ai.tracing.enabled=true");
    }
    if (traceApiKey === modelApiKey) {
      throw new Error("AI_MAINTAINER_TRACE_API_KEY must differ from AI_MAINTAINER_MODEL_API_KEY when ai.tracing.enabled=true");
    }
  ...
  try {
    ...
    const runner = new sdk.Runner({
      ...
      workflowName: `AI Repo Maintainer: ${mode}`
    });
  ...
  throw new Error(`AI maintainer ${mode} agent failed after ${agent.maximumAttempts} attempt(s): ${lastError?.message ?? lastError}`);
}
