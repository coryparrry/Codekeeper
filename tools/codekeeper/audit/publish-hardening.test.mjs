...
test("a failed automatic repair dispatch does not consume its retry marker", async () => {
  ...
  const restoreGitHub = replaceGitHubMethods({
    ...
    async addLabels(_number, labelsToAdd) {
      ...
      this.advancePullMutationState(
        "POST",
        this.repoPath(`/issues/${context.pullRequest.number}/labels`),
        { labels: labelsToAdd }
      );
      if (failAddAfterMutation) return;
    },
    async rollbackPullLabel(_number, label) {
      ...
      this.advancePullMutationState(
        "DELETE",
        this.repoPath(`/issues/${context.pullRequest.number}/labels/${encodeURIComponent(label)}`)
      );
    },
  ...
  try {
    ...
    const recoveredAdd = await publishReview({
      artifactDirectory,
      config: reviewConfig,
      configSha256,
      agentProfilePath: profilePaths.review,
      ...integrity,
      token: "unused"
    });
    assert.deepEqual({
