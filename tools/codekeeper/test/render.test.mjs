...
import { normalizeReleaseOwnedPinReview, renderIssueTriage, renderReviewComment, sanitizeMarkdown } from "../src/lib/render.mjs";
...
test("installer pin-only updates keep live validation release-owned", () => {
  const oldSha = "1".repeat(40);
  const newSha = "2".repeat(40);
  const files = ["assistant", "fix", "issues", "maintain", "review"].map((mode) => ({
    filename: `.github/workflows/codekeeper-${mode}.yml`,
    additions: 3,
    deletions: 3,
    patch: [
      `-# Generated by codekeeper from coryparry/Codekeeper@${oldSha}.`,
      `+# Generated by codekeeper from coryparry/Codekeeper@${newSha}.`,
      `-      - uses: coryparry/Codekeeper/tools/codekeeper@${oldSha}`,
      `+      - uses: coryparry/Codekeeper/tools/codekeeper@${newSha}`,
      `-    uses: coryparry/Codekeeper/.github/workflows/codekeeper-${mode}.yml@${oldSha}`,
      `+    uses: coryparry/Codekeeper/.github/workflows/codekeeper-${mode}.yml@${newSha}`,
    ].join("\n"),
  }));
  const modelResult = {
    summary: "Manual validation is still required.",
    tests: { adequate: false, notes: "Run the workflows manually.", missingTest: null },
  };
  const result = normalizeReleaseOwnedPinReview(modelResult, files);
  assert.match(result.summary, /installer-generated update advances the same reviewed Codekeeper source pin/);
  assert.match(result.tests.notes, /no adopter test is required/);
  assert.doesNotMatch(`${result.summary} ${result.tests.notes}`, /manual validation|run the workflows manually/i);
  assert.equal(normalizeReleaseOwnedPinReview(modelResult, [...files.slice(0, 4), files[0]]), modelResult);
});
