...
test("release publication is tag-gated, provenance-enabled, and bound to a protected npm environment", async () => {
  ...
  assert.match(source, /actions\/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1 # v7\.0\.1/);
  assert.match(source, /actions\/setup-node@820762786026740c76f36085b0efc47a31fe5020 # v7\.0\.0/);
  assert.match(source, /actions\/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a # v7\.0\.1/);
  assert.match(source, /actions\/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c # v8\.0\.1/);
  assert.match(source, /actions\/attest-build-provenance@1e69f48acb82d1966a394da916b4c1698aa569d6 # v4/);
  assert.match(source, /node-version: 24\.19\.0/);
  assert.match(source, /npm install --global npm@12\.0\.2 --ignore-scripts --no-audit --no-fund/);
...
test("release workflow only publishes a locally reverified tarball and rechecks the public registry receipt", async () => {
  ...
  assert.match(source, /--expected-manifest-sha256 "\$EXPECTED_MANIFEST_SHA256"/);
  assert.match(source, /--expected-source-commit "\$EXPECTED_SOURCE_COMMIT"/);
  assert.match(source, /npm publish "\$TARBALL" --access public --provenance --ignore-scripts/);
  assert.match(source, /npm view "codekeeper@\$EXPECTED_VERSION" version dist\.integrity dist\.tarball --json/);
  ...
  assert.match(source, /NPM_CONFIG_USERCONFIG: \$\{\{ runner\.temp \}\}\/codekeeper-public-npmrc/);
  assert.match(source, /Run fresh exact-version install canary/);
  assert.match(source, /npm install --ignore-scripts --no-audit --no-fund "codekeeper@\$EXPECTED_VERSION"/);
  ...
  assert.match(source, /gh release create "\$RELEASE_TAG" "\$TARBALL" "\$sidecar"/);
