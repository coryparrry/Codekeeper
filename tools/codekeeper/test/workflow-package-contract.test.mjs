...
test("review package verification is consolidated across two trusted boundaries", async () => {
  if (wrappersActive) {
    ...
    assert.equal(
      (genericRuntime.match(/Acquire exact Codekeeper package/g) ?? []).length,
      4,
    );
    assert.equal(
      (genericRuntime.match(/Install exact Codekeeper runtime/g) ?? []).length,
      3,
    );
    return;
  }
...
test("artifact boundaries receive each runner's verified package source identity", async () => {
  if (wrappersActive) {
    assert.equal(
      (
        genericRuntime.match(
          /steps\.codekeeper-package\.outputs\.source_commit/g,
        ) ?? []
      ).length >= 3,
      true,
    );
...
test("product workflows require fresh GitHub-hosted Ubuntu runners", async () => {
  if (wrappersActive) {
    assert.equal(
      (genericRuntime.match(/^    runs-on: ubuntu-latest$/gm) ?? []).length,
      4,
    );
    for (const mode of modes)
      assert.doesNotMatch(await workflow(mode), /runs-on:/);
    return;
  }
  ...
  if (wrappersActive) {
    ...
    assert.ok(
      compute.indexOf("--operation workspace") <
        compute.indexOf("CODEKEEPER_MODEL_API_KEY"),
    );
    assert.ok(
      compute.indexOf("--operation workspace") <
...
test("all checkouts discard persisted credentials and tool versions are exact", async () => {
  if (wrappersActive) {
    const checkoutCount = (
      genericRuntime.match(/uses: actions\/checkout@/g) ?? []
    ).length;
    assert.equal(
      (genericRuntime.match(/persist-credentials: false/g) ?? []).length,
      checkoutCount,
    );
    assert.equal(
      (genericRuntime.match(/node-version: 24\.19\.0/g) ?? []).length,
      4,
    );
    for (const action of [
      "actions/checkout",
