...
const baseInput = {
  ...
  request: { eventName: "issue_comment", targetNumber: 197, requestedBy: "coryparry" },
  repository: { defaultBranch: "main", baseSha: "b".repeat(40), headSha: "c".repeat(40) },
  digests: {
    modePlan: digest("1"),
    policy: digest("2"),
    profile: digest("3"),
...
function computeEnvelope() {
  ...
  return advanceEnvelope(
    createEnvelope({ ...baseInput, digests }),
    "compute-complete",
    { digests: { workspaceResult: sha256(values["workspace-result.json"]), candidate: sha256(values["candidate.json"]), validationReceipt: null } },
  );
}
...
function validationEnvelope() {
  ...
  return advanceEnvelope(
    computeEnvelope(),
    "validation-complete",
    { validationRequired: true, digests: { validationReceipt: sha256(values["validation-receipt.json"]) } },
  );
}

function noValidationEnvelope() {
  return advanceEnvelope(computeEnvelope(), "validation-not-required", { validationRequired: false });
}
...
async function createValidHandoff(envelope = computeEnvelope(), kind = "compute", options = {}) {
  ...
  await createHandoff({ directory, envelope, kind, files: payloads(envelope), ...options });
  return directory;
}
...
test("envelopes use an exact closed schema and canonical bytes", () => {
  ...
  assert.throws(() => { envelope.mode = "review"; }, TypeError);
  assert.throws(() => assertEnvelope({ ...envelope, extra: true }), /unexpected or missing/);
  assert.throws(() => parseEnvelope(JSON.stringify({ ...envelope, package: { ...envelope.package, name: "codekeeper" } })), /must be/);
});

test("envelope validation rejects malformed fields, unknown fields, and prototype tricks", () => {
  const cases = [
    ["schema", (value) => { value.schemaVersion = 2; }],
    ["state", (value) => { value.state = "skipped"; }],
    ["mode", (value) => { value.mode = "../fix"; }],
    ["run repository", (value) => { value.run.repository = "owner"; }],
    ["run id", (value) => { value.run.runId = ""; }],
    ["run attempt", (value) => { value.run.attempt = 0; }],
    ["package name", (value) => { value.package.name = "codekeeper"; }],
    ["package version", (value) => { value.package.version = "latest"; }],
    ["package integrity", (value) => { value.package.integrity = "sha256-bad"; }],
    ["package source", (value) => { value.package.sourceCommit = "short"; }],
    ["request event", (value) => { value.request.eventName = "IssueComment"; }],
    ["request target", (value) => { value.request.targetNumber = 0; }],
    ["request user", (value) => { value.request.requestedBy = "bad\nuser"; }],
    ["repository branch", (value) => { value.repository.defaultBranch = "../main"; }],
    ["repository base", (value) => { value.repository.baseSha = "short"; }],
    ["repository head", (value) => { value.repository.headSha = "short"; }],
    ["digest", (value) => { value.digests.modePlan = "bad"; }],
    ["unknown top field", (value) => { value.unknown = true; }],
    ["unknown nested field", (value) => { value.run.unknown = true; }],
    ["unknown digest field", (value) => { value.digests.unknown = true; }],
  ];
...
test("envelope transitions are monotonic and bind stage outputs", () => {
  ...
  assert.throws(() => advanceEnvelope(compute, "validation-complete", { validationRequired: true, digests: { validationReceipt: null } }), /requires a validation receipt/);
  assert.throws(() => advanceEnvelope(compute, "validation-not-required", { validationRequired: false, digests: { validationReceipt: digest("8") } }), /cannot contain/);
  const sealed = advanceEnvelope(validationEnvelope(), "sealed", { digests: {} });
  ...
  assert.throws(() => advanceEnvelope(compute, "validation-not-required", { validationRequired: false, digests: { modePlan: digest("9") } }), /changed during/);
  assert.throws(() => advanceEnvelope(compute, "validation-complete", { digests: { validationReceipt: digest("8") } }), /explicit validationRequired/);
  assert.throws(() => advanceEnvelope(compute, "validation-complete", { validationRequired: false, digests: { validationReceipt: digest("8") } }), /does not match/);
  assert.equal(advanceEnvelope(compute, "validation-not-required", { validationRequired: false }).state, "validation-not-required");
  assert.throws(() => advanceEnvelope(compute, "validation-not-required", { validationRequired: true }), /does not match/);
  assert.throws(() => advanceEnvelope(validationEnvelope(), "sealed", { validationRequired: true }), /only valid/);
  assert.throws(() => advanceEnvelope(compute, "validation-complete", { validationRequired: true, digests: null }), /plain object/);
});

test("handoff creation and verification bind exact files, state, run, and package", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "codekeeper-handoff-"));
  const envelope = computeEnvelope();
  const created = await createHandoff({ directory, envelope, kind: "compute", files: payloads(envelope) });
  ...
  const verified = await verifyHandoff({ directory, expectedEnvelope: envelope, expectedKind: "compute", expectedRun: envelope.run, expectedPackage: envelope.package });
  assert.equal(verified.envelopeSha256, created.envelopeSha256);
  assert.deepEqual(verified.files.map(({ path: filePath }) => filePath), [
    "candidate.json",
    "context.json",
    HANDOFF_ENVELOPE_FILE,
    "mode-plan.json",
...
test("validation-complete handoffs require the receipt and reject a replayed run", async () => {
  ...
  await createHandoff({ directory, envelope, kind: "validation", files: payloads(envelope) });
  await assert.doesNotReject(verifyHandoff(directory, { expectedState: "validation-complete" }));
  const replay = structuredClone(envelope);
  replay.run.runId = "different-run";
  await assert.rejects(verifyHandoff(directory, { expectedEnvelope: replay }), /does not match/);
});
...
test("all handoff kinds preserve their state contract, including same-run retries", async () => {
  ...
  for (const [kind, envelope, state] of cases) {
    ...
    const first = await verifyHandoff({ directory, expectedKind: kind, expectedState: state, expectedRun: envelope.run });
    const retry = await verifyHandoff({ directory, expectedKind: kind, expectedState: state, expectedRun: envelope.run });
  ...
  await assert.rejects(createHandoff({ directory: invalid, envelope: sealedEnvelope(), kind: "compute", files: payloads(sealedEnvelope()) }), /cannot carry/);
});
...
test("each bound payload digest independently rejects stale bytes", async () => {
  ...
  for (const [name, relativePath] of paths) {
    ...
    await assert.rejects(verifyHandoff(directory), /inventory|digest|stale/);
  }
  ...
  await assert.rejects(verifyHandoff(receiptDirectory), /inventory|digest|stale/);
  ...
  await assert.rejects(verifyHandoff(receiptEnvelopeDirectory), /envelope digest|stale/);
});
...
test("envelope and manifest tampering cannot be repaired without the trusted digest", async () => {
  for (const key of ["modePlan", "policy", "profile", "context", "workspaceResult", "candidate"]) {
    ...
    await assert.rejects(verifyHandoff(directory), /envelope digest|does not match|stale/);
  }
  const fields = [
    ["envelope hash", (manifest) => { manifest.envelopeSha256 = digest("f"); }],
    ["inventory hash", (manifest) => { manifest.inventorySha256 = digest("f"); }],
    ["file bytes", (manifest) => { manifest.files[0].bytes += 1; }],
    ["file digest", (manifest) => { manifest.files[0].sha256 = digest("f"); }],
    ["wrong run", (manifest) => { manifest.run.runId = "other-run"; }],
    ["wrong attempt", (manifest) => { manifest.run.attempt = 2; }],
    ["wrong state", (manifest) => { manifest.state = "sealed"; }],
    ["wrong kind", (manifest) => { manifest.kind = "sealed"; }],
    ["unknown top field", (manifest) => { manifest.extra = true; }],
    ["unknown run field", (manifest) => { manifest.run.extra = true; }],
    ["unknown file field", (manifest) => { manifest.files[0].extra = true; }],
    ["duplicate", (manifest) => { manifest.files.push(structuredClone(manifest.files[0])); }],
    ["unsorted", (manifest) => { manifest.files.reverse(); }],
  ];
  for (const [name, mutate] of fields) {
    const directory = await createValidHandoff();
    await rewriteManifest(directory, mutate);
    await assert.rejects(verifyHandoff(directory), /mismatch|stale|unexpected|duplicate|sorted|does not match|cannot carry|digest/ , name);
  }
});
...
test("handoff file sets reject missing, extra, duplicate, and optional patch drift", async () => {
  ...
  await assert.rejects(verifyHandoff(missing), /mismatch|missing|digest/);

  const extra = await createValidHandoff();
  await writeFile(path.join(extra, "unexpected.json"), "unexpected");
  await assert.rejects(verifyHandoff(extra), /mismatch|unexpected/);

  const withPatch = await mkdtemp(path.join(os.tmpdir(), "codekeeper-handoff-"));
  const envelope = computeEnvelope();
  const expectedFiles = ["candidate.json", "context.json", HANDOFF_ENVELOPE_FILE, "mode-plan.json", "patch.diff", "policy.json", "profile.json", "workspace-result.json"];
  await createHandoff({ directory: withPatch, envelope, kind: "compute", files: { ...payloads(envelope), "patch.diff": Buffer.from("patch") }, expectedFiles });
  await assert.doesNotReject(verifyHandoff({ directory: withPatch, expectedFiles }));
});
...
test("unsafe paths, hidden entries, links, empty directories, and special files fail closed", async () => {
  ...
  await assert.rejects(createHandoff({ directory: createRootLink, envelope: computeEnvelope(), kind: "compute", files: payloads(computeEnvelope()) }), /root|regular/);

  const emptyDirectory = await createValidHandoff();
  await mkdir(path.join(emptyDirectory, "empty"));
  await assert.rejects(verifyHandoff(emptyDirectory), /empty directory/);
  ...
  await assert.rejects(verifyHandoff(reservedEnvelope), /regular/);
  ...
  await assert.rejects(verifyHandoff(reservedManifest), /regular/);
  ...
  try {
    await assert.rejects(verifyHandoff(socketDirectory), /regular|only regular/);
  } finally {
...
test("handoff identity checks reject wrong run, attempt, state, package, and source", async () => {
  const envelope = computeEnvelope();
  const directory = await createValidHandoff(envelope);
  await assert.rejects(verifyHandoff({ directory, expectedRun: { ...envelope.run, runId: "wrong" } }), /run/);
  await assert.rejects(verifyHandoff({ directory, expectedRun: { ...envelope.run, attempt: 2 } }), /run/);
  await assert.rejects(verifyHandoff({ directory, expectedState: "sealed" }), /state/);
  await assert.rejects(verifyHandoff({ directory, expectedKind: "sealed" }), /kind/);
  await assert.rejects(verifyHandoff({ directory, expectedPackage: { ...envelope.package, version: "9.9.9" } }), /package/);
  await assert.rejects(verifyHandoff({ directory, expectedSourceCommit: "d".repeat(40) }), /source/);
});
...
test("inventory ordering is deterministic and manifest outputs are immutable", async () => {
  ...
  const created = await createHandoff({ directory, envelope, kind: "compute", files: reversed });
  assert.deepEqual(created.manifest.files.map(({ path: filePath }) => filePath), [...created.manifest.files.map(({ path: filePath }) => filePath)].sort());
  assert.throws(() => { created.manifest.files[0].path = "changed"; }, TypeError);
  const verified = await verifyHandoff(directory);
...
test("handoffs reject tampering, extra files, unsafe paths, links, and special entries", async (t) => {
  const cases = [
    ...
    ["unsafe payload", async (directory, envelope) => createHandoff({ directory: path.join(directory, "unsafe"), envelope, kind: "compute", files: { "../escape": "x" } }), /unsafe/],
  ...
  for (const [name, mutate, expected] of cases) {
    await t.test(name, async () => {
      const directory = await mkdtemp(path.join(os.tmpdir(), "codekeeper-handoff-"));
      const envelope = computeEnvelope();
      await createHandoff({ directory, envelope, kind: "compute", files: payloads(envelope) });
      if (name === "unsafe payload") {
        await assert.rejects(mutate(directory, envelope), expected);
      } else {
        await mutate(directory, envelope);
        await assert.rejects(verifyHandoff(directory), expected);
      }
    });
  }
});
