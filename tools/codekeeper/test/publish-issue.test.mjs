...
test("issue publication does not close a duplicate after a concurrent user comment", async () => {
  const artifactDirectory = await mkdtemp(
    path.join(os.tmpdir(), "codekeeper-issue-stale-test-"),
  );
  ...
  const context = {
    ...
    configSha256,
    runId: "7002",
    runUrl: "https://github.com/owner/repository/actions/runs/7002",
  ...
  const restoreGitHub = replaceGitHubMethods({
    async getIssue(number) {
      if (number === 9)
        return { number, state: "open", updated_at: "2026-08-05T09:00:00Z" };
      return {
        number,
        title: "Report",
        state: "open",
...
${marker}`,
    ...
    async createComment(_number, body) {
      duplicateCommentPublished ||= body.includes("Closing as a duplicate");
    },
    async updateIssue() {
      issueClosed = true;
    },
  ...
  try {
    ...
    if (previousId === undefined)
      delete process.env.CODEKEEPER_AUTOMATION_BOT_ID;
    else process.env.CODEKEEPER_AUTOMATION_BOT_ID = previousId;
    await rm(artifactDirectory, { recursive: true, force: true });
  }
});

test("issue publication publishes issue-specific needs-tests and preserves a trusted deferred marker", async () => {
  const artifactDirectory = await mkdtemp(
    path.join(os.tmpdir(), "codekeeper-issue-label-revision-test-"),
  );
  ...
  const context = {
    ...
    configSha256,
    runId: "7003",
    runUrl: "https://github.com/owner/repository/actions/runs/7003",
    issue: { number: 7, title: "Report", updatedAt: "2026-08-05T10:00:00Z" },
  };
  const result = {
    mode: "issue",
    ...
    type: "bug",
    priority: "p3",
    labels: [LABELS.ISSUE_NEEDS_TESTS],
    actionable: true,
    missingInformation: [],
    duplicateOf: null,
  ...
  let labels = [
    ...
    { name: "priority p1" },
    { name: "deferred" },
    { name: "codekeeper:needs-information" },
  ];
...
${deferredReviewMarker("f".repeat(64))}`,
  ...
  try {
    ...
    assert.deepEqual(modes, ["issue", "lifecycle"]);
    assert.deepEqual(
...
test("issue publication adds the managed needs-information label from triage", async () => {
  const artifactDirectory = await mkdtemp(
    path.join(os.tmpdir(), "codekeeper-issue-needs-information-test-"),
  );
  ...
  const context = {
    ...
    configSha256,
    runId: "7013",
    runUrl: "https://github.com/owner/repository/actions/runs/7013",
  ...
  const result = {
    ...
    labels: [],
    actionable: false,
    missingInformation: ["Which version contains this behavior?"],
    duplicateOf: null,
    duplicateConfidence: "none",
    implementationRecommendation: "manual",
    decision: {
  ...
  const restoreGitHub = replaceGitHubMethods({
    async beginIssueMutation() {
      return { number: 7, title: "Report", state: "open", labels: [] };
    },
  });
  const previousLogin = process.env.CODEKEEPER_AUTOMATION_BOT_LOGIN;
  const previousId = process.env.CODEKEEPER_AUTOMATION_BOT_ID;
