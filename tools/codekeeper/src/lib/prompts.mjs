...
For every finding, try to disprove it against the current head. Set classification to current, stale, already-fixed, pre-existing, preference-only, or not-actionable. Record the evidence in validation and name the deterministic prevention test. Add 1-8 stable lowercase rootCauseTags describing the concrete failure mechanism (for example, path-traversal or missing-authorization-check); keep them unique and do not use prose, severity, or repository labels as tags. Set reproductionTest to the exact existing repository-relative test path only when that test reproduces the finding; otherwise set it to null. Never guess a test path and never use a proposed or missing test as reproductionTest. Only a current, validated, actionable finding may block or pass to the Fixer. Severity measures impact; blocking measures merge readiness, so a low-severity finding must still block when this pull request introduces a reproducible contract violation that requires repair before merge. Use confidence=low when evidence is incomplete and keep that finding non-blocking. Assess whether the changed behavior has adequate deterministic tests. Set tests.missingTest only for a test that can be added to and run in the current checkout without fetching or executing external source. Name the repository-local test target, exact trigger or input, expected observable behavior, and the changed behavior it would fail before and pass after. If a change only advances immutable dependency, action, or reusable-workflow references whose source is not in the checkout, set tests.missingTest to null and record the evidence boundary neutrally in tests.notes. For a Codekeeper installer-generated setup or update that only advances its pinned source consistently, release acceptance owns live and provenance validation; never assign that validation to the adopter. Never request a generic local test of imported external behavior. An evidence gap is not missing coverage in this pull request. Auto-merge may be recommended only for a genuinely low-risk, mechanically safe change.
...
Set diagram only when the pull request makes a substantial change to a multi-step flow or state transition and a visual is materially clearer than the findings. Otherwise set diagram to null. When used, it must start exactly with graph LR and use only a left-to-right layout. Never use TD, TB, BT, RL, sequence, state, class, entity-relationship, Gantt, pie, mindmap, timeline, or Git graph diagrams. Keep it to four nodes or fewer. Do not diagram the review process itself. Do not add Markdown fences, links, clicks, initialization directives, or styling.
...
export function buildCoordinatorPrompt(mode, context, config) {
  ...
  switch (mode) {
    case "issue":
      ...
      break;
    case "review":
      action = "Decide whether the workspace review evidence supports blocking, manual review, or auto-merge. Findings must be copied exactly from the workspace evidence, including rootCauseTags and reproductionTest; do not normalize, add, remove, or infer either field. Every specialist blocking finding must remain blocking; non-blocking findings may be omitted or retained only as non-blocking. Copy every feedback group and its evidence exactly. Keep fix_now and fix_if_cheap dispositions unchanged because they carry merge and repair authority. A defer disposition may stay defer or move to ignore; keep ignore unchanged. Never upgrade a disposition, invent a feedback group, omit a feedback group, or alter its evidence.";
      break;
    case "audit":
