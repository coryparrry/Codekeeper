...
`review.reasoningEscalation` keeps ordinary reviews on their configured agent and routes exceptional reviews to a stronger provider/model/effort tuple. The starter uses OpenAI Luna Max when a pull request has the `security` or `risk high` label, matches a configured security/high-risk path, changes at least 5,000 lines, or concentrates at least 1,000 changed lines in one file. The decision is calculated from the exact frozen comparison and applies to both the coordinator and read-only workspace specialist. Disable the object with `enabled=false` or adjust its labels, glob patterns, thresholds, provider, model, and effort to fit the repository. Lower thresholds increase latency and cost because the complete review runs at the escalated tier.
