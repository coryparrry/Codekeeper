...
The escalation tier is Max rather than High. By explicit product decision, it ships without a separate Max evaluation: the frozen review routes both the coordinator and read-only workspace specialist to Luna Max when the PR already carries `security` or `risk high`, touches a configured security/high-risk path, changes at least 5,000 lines, or changes at least 1,000 lines in one file. All other reviews remain on the measured Luna Medium configuration, so the latency cost is isolated to the exceptional tier. High remains neither a production tier nor a proxy for Max. This routing decision is recorded in frozen context and runtime metadata; it is a product policy choice, not a measured claim that Max improves the Qodo scores above.
