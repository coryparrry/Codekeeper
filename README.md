...
The guided installer performs this generation from release-pinned assets. Until it is published, use a locally built installer tarball for private acceptance or follow the [manual installation guide](INSTALL.md). The generated setup includes [`.github/codekeeper.json`](.github/codekeeper.json), selected callers, and local bootstrap/runtime workflows. One strict artifact catalog owns every managed repository payload destination and its update, rename, or retirement rule; `.github/codekeeper-release.json` is the separate control record that inventories that payload. New copied Markdown, configuration, or workflow assets join the catalog instead of requiring another reconciliation branch. Agent profiles come from the verified package by default; an adopter file under `.github/codekeeper/agents/` exists only when a maintainer opts into a repository-specific override. Each caller pins one exact npm version and SHA-512 integrity. The local bootstrap verifies that tarball, its closed file manifest, and source provenance before any runtime dependency is installed. Adopters do not copy `tools/codekeeper` or runtime dependencies into their repository, and do not provide a source-repository token. The manual source-pinned path remains a compatibility option for unpublished/private acceptance.
...
- A local bootstrap downloads the caller's exact npm package receipt and independently verifies the tarball SHA-512 before trusting its closed manifest. Every reusable job re-verifies the run-scoped package artifact, exact inventory, hashes, source commit, and absence of symlinks or hidden paths before it runs Codekeeper. Adopter policy is read only from its default branch.
