...
Profiles tune priorities, work selection, implementation approach, review standards, and reporting. Capability switches control repair, issue implementation, issue closure, and merge actions. Profiles cannot enable a disabled capability, expand allowed paths, bypass protected paths or validation, change the target, expose credentials, or grant tools and network access. In Settings, press `R` on a profile to remove an existing repository override and return to the packaged default; later releases then advance that profile automatically.
