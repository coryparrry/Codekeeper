...
It is not a hosted service, webhook receiver, or multi-tenant GitHub App. The release builder stages the CLI, TUI, production runtime, agents, integrations, reusable workflows, and their exact dependencies as one closed [`codekeeper` package](packages/codekeeper/README.md). The package remains unpublished while private acceptance is in progress. New and updated installations pin that package by exact version and npm SHA-512 integrity; installations generated by older releases keep using their immutable compatibility commit until their update pull request merges. The CLI does not copy runtime source or dependencies into adopter repositories. Each adopter owns its GitHub App credentials and policy.
...
The guided installer performs this generation from release-pinned assets. Until it is published, use a locally built installer tarball for private acceptance or follow the [manual installation guide](INSTALL.md). The generated setup includes [`.github/codekeeper.json`](.github/codekeeper.json), selected callers, one local package-acquisition action, and local runtime workflows. One strict artifact catalog owns every managed repository payload destination and its update, rename, or retirement rule; `.github/codekeeper-release.json` is the separate control record that inventories that payload. New copied Markdown, configuration, or workflow assets join the catalog instead of requiring another reconciliation branch. Agent profiles come from the verified package by default; an adopter file under `.github/codekeeper/agents/` exists only when a maintainer opts into a repository-specific override. Each caller pins one exact npm version and SHA-512 integrity. The first runtime job downloads and verifies that exact tarball, then shares it as a run-scoped artifact that every later isolated job reverifies before installing runtime dependencies. Adopters do not copy `tools/codekeeper` or runtime dependencies into their repository, and do not provide a source-repository token. The manual source-pinned path remains a compatibility option for unpublished/private acceptance.
