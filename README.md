...
It is not a hosted service, webhook receiver, or multi-tenant GitHub App. The release builder stages the CLI, TUI, production runtime, agents, presets, reusable workflows, and their exact dependencies as one closed [`codekeeper@0.2.0` package](packages/codekeeper/README.md), published on npm. New and updated installations pin that package by exact version and npm SHA-512 integrity; installations generated by older releases keep using their immutable compatibility commit until their update pull request merges. The CLI does not copy runtime source or dependencies into adopter repositories. Each adopter owns its GitHub App credentials and policy.
...
| `codekeeper-issues.yml` | Triage opened, reopened, edited, or closed issues while `auto_triage=true`; closed events finalize resolution after a merged repair, and configured-owner `/codekeeper triage` comments remain available when automatic triage is off. |
...
The guided installer performs this generation from release-pinned assets. Start from a clean adopter checkout with `npx --yes codekeeper@0.2.0 init`, or follow the [installation guide](INSTALL.md). The generated setup includes [`.github/codekeeper.json`](.github/codekeeper.json), selected callers, one local package-acquisition action, and local runtime workflows. One strict artifact catalog owns every managed repository payload destination and its update, rename, or retirement rule; `.github/codekeeper-release.json` is the separate control record that inventories that payload. New copied Markdown, configuration, or workflow assets join the catalog instead of requiring another reconciliation branch. Agent profiles come from the verified package by default; an adopter file under `.github/codekeeper/agents/` exists only when a maintainer opts into a repository-specific override. Each caller pins one exact npm version and SHA-512 integrity. The first runtime job downloads and verifies that exact tarball, then shares it as a run-scoped artifact that every later isolated job reverifies before installing runtime dependencies. Adopters do not copy `tools/codekeeper` or runtime dependencies into their repository, and do not provide a source-repository token. Source-pinned installation is retained only for maintainer recovery and historical compatibility.
...
- Each workflow run downloads the caller's exact npm package once. The first runtime job verifies the registry receipt, tarball SHA-512, closed manifest, exact inventory, hashes, source commit, and absence of links or hidden paths, then uploads a one-day run-scoped artifact. Every later isolated job downloads and independently reverifies that artifact before it runs Codekeeper. There is no separate serial package-bootstrap job. Adopter policy is read only from its default branch.
