...
test("audit publication rechecks the remote default branch immediately before mutation", () => {
  const auditPublication = section(publish, "export async function publishAudit", "export async function publishFix");
  assertContains(auditPublication, /github\.(getBranch|defaultBranchRevision)/u, "audit publication relies on the stale local checkout tip");
});
